    ...
    (err,result)=>{
        if(result.length){
            return res.status(409).send({
                msg:'Este nombre usuario ya esta en uso.'
            });
        }else{
            //username is avaible
            bcrypt.hash(req.body.password, 10,(err,hash)=>{
                if(err){
                    ...
                    //has hashed password => added to db
                    db.query(`INSERT INTO users (id, username, password, registered) VALUES
                    (
                        '${uuid.v4()}',
    ...
    (err,result)=>{
        ...
        if(!result.length){
            return res.status(401).send({
                ...
                //Nunca especificar cual esta mal!!!!!!
            });
        }
